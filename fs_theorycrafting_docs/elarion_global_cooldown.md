# Elarion's Global Cooldown: the formula, no wall, and what's free to weave
...
Cast bars and the Heartseeker Barrage channel scale by the same `1 / (1 + haste)`, so as your GCD tightens, so do your casts. Haste isn't doing two different things to your rotation. It's doing one thing everywhere.
...
## What's off the global — weave it for free

Not everything you press spends a global. These don't, and you can fire them inside the GCD without losing a cast:

- Lunarlight Mark
- Disrupt (your interrupt)
- Roll
- Your defensives (Sanctuary, Pathfinder's Resilience, Major Invisibility)
- Auto-attacks

Mark is the one that matters most for damage. Applying it costs you nothing on the global, so there's no reason to treat marking as a turn in your rotation — slip it between casts and your damage abilities keep flowing. Same idea with the interrupt and a defensive: pressing them doesn't push your next shot back.

## What to do with this

Haste tightens your whole cast cadence with no wall in front of it — but it's a curve, not a flat multiplier. There's no breakpoint where it dies, so it never becomes a dead stat. Early haste buys real global; once you're already fast, each extra point shaves less, so at the top end more haste tends to do more for your damage elsewhere than for the global itself. And stop spending mental budget worrying that Mark or an interrupt is eating a global. It isn't. The things you'd want off the GCD already are.
